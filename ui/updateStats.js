...
export function updateStats(todos, archivedTodos) {
 ...
 if (!Array.isArray(todos) || !Array.isArray(archivedTodos)) {
        console.warn('updateStats: одни из данных не массив', { todos, archivedTodos });
        return;
    }

    const activeTasks = todos.length;
    const completedTasksCount = archivedTodos.length;
